...
void Texture::render(const Camera&camera,float x, float y, float w, float h, int spriteIndex, float angle,bool horizontalFlip)
{
	...
	renderQuad.x = 	(int)(x*(camera.getSize().w / WINDOWSIZE_W));
	renderQuad.y = 	(int)(y*(camera.getSize().h / WINDOWSIZE_H));
	renderQuad.w = 	(int)(w*(camera.getSize().w / WINDOWSIZE_W) + ERROR_DISPLAY);
	renderQuad.h = 	(int)(h*(camera.getSize().h / WINDOWSIZE_H) + ERROR_DISPLAY);
...
void Texture::render(int x, int y,SDL_Rect*desRect)
{
	...
	renderQuad.x = x;
	renderQuad.y = y;
	renderQuad.w = (int)size->w;
	renderQuad.h = (int)size->h;
...
{
	SDL_Rect renderQuad;
	renderQuad.x = (int)location->x- (int)size->w/2;
	renderQuad.y = (int)location->y- (int)size->h/2;
	renderQuad.w = (int)size->w;
	renderQuad.h = (int)size->h;
...
//angle is in degreee.
{
	SDL_Rect renderQuad;
	if (size != NULL){
		renderQuad.x = x-(int)size->w/2;
		renderQuad.y = y - (int)size->h/2;
		renderQuad.w = (int)size->w;
		renderQuad.h = (int)size->h;
	}
	else {
	
		renderQuad.x = x- (int)this->size->w/2;
		renderQuad.y=y - (int)this->size->h/2 ;
		renderQuad.w = (int)this->size->w;
		renderQuad.h = (int)this->size->h;
	}
