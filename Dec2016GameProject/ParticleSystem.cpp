...
ParticleSystem::ParticleSystem(float x,float y, float velX,float velY, float size,
	 Texture * texture, int lifespan,int speedOfLifespan)
	 :Particle(x,y,velX,velY,size,texture,lifespan,speedOfLifespan)
{
}
...
void ParticleSystem::putParticle()
{
	...
	Particle*newParticle = new Particle(
		...
		maxSpeed*cosf(angle), maxSpeed*sinf(angle),size.w
		,texture, 255, 20);
